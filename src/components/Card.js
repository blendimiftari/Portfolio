      ...
      {img ? (
        <div className="sm:col-span-2">
          <img
            alt=""
            src={img}
            className="rounded-md border-4 border-gray-600 object-cover w-32 h-20"
          />
        </div>
      ) : (
        <div className="text-paragraph text-xs pt-1 font-semibold uppercase tracking-wide sm:col-span-2">
          <p>{dates}</p>
        </div>
      )}
      <div className="sm:col-span-4">
        <h3 className="text-base flex group-hover:text-violet-300 font-bold text-textPrimary mb-2">
          {title}
          {link && (
            <MdArrowOutward
        ...
        <h2 className="text-sm font-semibold text-paragraph mb-2">
          {subtitle}
        </h2>
        <p className="text-paragraph text-sm leading-relaxed mb-4">{content}</p>
